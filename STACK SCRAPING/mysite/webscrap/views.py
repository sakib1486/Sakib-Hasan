...
def newest(request) :
    ...
    for i in soup.select('.question-hyperlink'):
        ques[Count] = i.text
        link[Count] = "https://www.stackoverflow.com" + i.get('href')
        Count += 1
        if Count == 10:
            break

    return render(request, 'webscrap/newest.html', {'res': zip(link, ques)})
    #return render(request, 'webscrap/newest.html', {'ques' : [ques], 'link' : [link] })


#Most voted questions view
def voted(request) :
    res = requests.get('https://stackoverflow.com/search?tab=newest&q=android%20features')  #Data from Stack Overflow
    ...
    return render(request, 'webscrap/voted.html', {'res': zip(link, ques)})